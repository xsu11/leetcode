...
public class Q28_FindTheIndexOfTheFirstOccurrenceInAString {
    ...
    private int[] solve(char[] p) {
        ...
        for (int i = 2; i < p.length; i++) {
            ...
            if (lastMatchCount == 0) {
                ...
                // lastMatchCount == 0 || p[lastMatchCount] == p[i]
                // set pi[i]
                if (lastMatchCount == 0) {
                    pi[i] = p[0] == p[i] ? 1 : 0;
                } else if (p[lastMatchCount] == p[i]) {
    ...
    private int check(char[] s, char[] p, int[] pi) {
        // check match of s[0] and p[0]
        int matchCount = p[0] == s[0] ? 1 : 0;
        // matchCount is 0(not match) or 1(match)

        // boundary case
        if (p.length == 1 && matchCount == 1) {
            // matchCount == 1(match) && p.length == 1, this means p[0] == s[0], found the first occurrence
            return 0;
        }
        if (s.length == 1) {
            // already finish checking s, matchCount indicate whether we found a match or not
        ...
        // either: p.length == 1, but s[0] does not match p[0], need to check further
        // or: p.length > 1, no matter s[0] and p[0] is a match or not, need to check further

        // check match count for s[1:s.length -1] with p
        for (int i = 1; i < s.length; i++) {
            // initially matchCount is 0 or 1 after check s[] with p[0], check both cases
            ...
            if (matchCount == 0) {
                 ...
                 */
                while (matchCount > 0 && s[i] == p[matchCount]) {
