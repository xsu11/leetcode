package com.xinsu;

/*
 * There is a row of n houses, where each house can be painted one of three colors: red, blue, or green. The cost of
 * painting each house with a certain color is different. You have to paint all the houses such that no two adjacent
 * houses have the same color.
 *
 * The cost of painting each house with a certain color is represented by an n x 3 cost matrix costs.
 *
 * For example, costs[0][0] is the cost of painting house 0 with the color red; costs[1][2] is the cost of painting
 * house 1 with color green, and so on...
 * Return the minimum cost to paint all houses.
 *
 *
 *
 * Example 1:
 *
 * Input: costs = [[17,2,17],[16,16,5],[14,3,19]]
 * Output: 10
 * Explanation: Paint house 0 into blue, paint house 1 into green, paint house 2 into blue.
 * Minimum cost: 2 + 5 + 3 = 10.
 * Example 2:
 *
 * Input: costs = [[7,6,2]]
 * Output: 2
 *
 *
 * Constraints:
 *
 * costs.length == n
 * costs[i].length == 3
 * 1 <= n <= 100
 * 1 <= costs[i][j] <= 20
 */

public class Q256_PaintHouse {

    public int minCost(int[][] costs) {
        // boundary case
        if (costs.length == 0) {
            return 0;
        }

        int red_1 = costs[0][0];
        int blue_1 = costs[0][1];
        int green_1 = costs[0][2];

        int red = red_1;
        int blue = blue_1;
        int green = green_1;

        for (int i = 1; i < costs.length; i++) {
            /*
             * red(i) = min(blue(i-1), green(i-1)) + cost[i][0]
             * blue(i) = min(red(i-1), green(i-1)) + cost[i][1]
             * green(i) = min(red(i-1), blue(i-1)) + cost[i][2]
             */
            red = Math.min(blue_1, green_1) + costs[i][0];
            blue = Math.min(red_1, green_1) + costs[i][1];
            green = Math.min(red_1, blue_1) + costs[i][2];

            // update red_1/blue_1/green_1
            red_1 = red;
            blue_1 = blue;
            green_1 = green;
        }

        return Math.min(Math.min(red, blue), green);
    }

}
