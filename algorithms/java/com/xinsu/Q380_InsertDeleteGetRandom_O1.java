...
public class Q380_InsertDeleteGetRandom_O1 {

    private final Map<Integer, Integer> locations;
    private final List<Integer> nums;

    public Q380_InsertDeleteGetRandom_O1() {
        this.locations = new HashMap<>();
        this.nums = new ArrayList<>();
    }

    public boolean insert(int val) {
        if (this.locations.containsKey(val)) {
            return false;
        }
    ...
    public boolean remove(int val) {
        ...
        final int location = this.locations.get(val);
        if (location == this.nums.size() - 1) {
            // val at last pos, remove only
            removeLast();
        } else {
            // val before last pos, swap with last pos ele then remove
            swap(location, this.nums.size() - 1);
            removeLast();
        }
    ...
    public int getRandom() {
        ...
        return this.nums.get(r.nextInt(this.nums.size()));
    }

    private void removeLast() {
        this.locations.remove(this.nums.get(nums.size() - 1));
        this.nums.remove(nums.size() - 1);
    }

    private void swap(final int i, final int j) {
        this.locations.put(this.nums.get(j), i);
        this.locations.put(this.nums.get(i), j);

        final int t = this.nums.get(i);
        this.nums.set(i, this.nums.get(j));
        this.nums.set(j, t);
    }
