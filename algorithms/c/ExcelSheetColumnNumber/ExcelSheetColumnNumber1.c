...
int excelSheetTitleToNumber(char *s, int pos) {
	if (s == NULL || s[0] == '\0') {
		return 0;
	} else if (pos < 0) {
		return 0;
	...
	return 26 * excelSheetTitleToNumber(s, pos - 1) + (int) (s[pos] - 'A') + 1;
}

int titleToNumber(char *s) {
	return excelSheetTitleToNumber(s, strlen(s) - 1);
}
