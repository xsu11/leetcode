...
void getCombinationSum2(int *a, int aSize, int target, struct Sll *sll,
		struct Sll *list) {
	// base case
	if (target < 0) {
		return;
	...
	// This for loop tries all possible combinations and the rests
	// by filling the current position with every element in a.
	for (int i = 0; i < aSize && a[i] <= target; i++) {
		// each one loop will try every possible combinations by filling current pos with a[i]

		// if i > 0, it means you have tried all possible combination with current pos filled with a[i - 1],
		// then if a[i - 1] == a[i], we don't need to fill current pos with a[i] and
		// try all possible combinations for the rests because we have done that in previou loop
		if (i > 0 && a[i - 1] == a[i]) {
			continue;
		}
		...
		getCombinationSum2(a + i + 1, aSize - i - 1, target - a[i], sll, list);
...
int **combinationSum2(int *candidates, int candidatesSize, int target,
	...
	getCombinationSum2(a, candidatesSize, target, sll, list);
